...
ppp_generic_setup() {
	...
	if [ "${demand:-0}" -gt 0 ]; then
		...
		demand_route=1
		demand_conf=""
		demand_conf="precompiled-active-filter /etc/ppp/filter demand idle $demand persist"
		if [ "$ipv6" = 1 ]; then
			...
			demand_conf="precompiled-active-filter /etc/ppp/ipv6filter demand idle $demand persist"
		else
