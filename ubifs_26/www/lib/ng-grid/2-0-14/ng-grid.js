...
ngAggregate.prototype.notifyChildren = function () {
    ...
    for (var i = 0; i < longest; i++) {
        ...
        if (i > this.aggIndex && this.rowFactory.aggCache[i]) {
            ...
            var offset = (22 * this.children.length);
...
var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
    ...
    $scope.pagingOnTop = self.config.pagingOnTop;
    ...
    $scope.topPanelHeight = function() {
        var height = self.config.showGroupPanel === true ? self.config.headerRowHeight + 32 : self.config.headerRowHeight;
        return height + (self.config.pagingOnTop ? 31 : 0);  //31px for ngPagerContainer
    };

    $scope.viewportDimHeight = function() {
        return Math.max(0, self.rootDim.outerHeight - $scope.topPanelHeight() - $scope.footerRowHeight + ((self.config.pagingOnTop ? 0 : 0)) );
    };
  ...
  // function to manage the selection action of a data item (entity)
    ...
    self.toggleSelectAll = function (checkAll, bypass, selectFiltered) {
        ...
        if (bypass || grid.config.beforeSelectionChange(rows, checkAll)) {
            ...
            for (var i = 0; i < rows.length; i++) {
				//add condition for default true
				if(!rows[i].default_value){
					wasSelected = rows[i].selected;
					rows[i].selected = checkAll;
				}
                if (rows[i].clone && !rows[i].default_value) {
                            ...
                            var dataWatcher = function (a) {
                                ...
                                angular.forEach(grid.data, function (item, j) {
                                    ...
                                    if (grid.rowCache[indx]) {
										...
										grid.rowCache[indx].default_value = item.default_value;
                                    }
                                    grid.rowMap[indx] = j;
                                });
...
angular.module('ngGrid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('checkboxCellTemplate.html',
    "<div class=\"ngSelectionCell\"><input tabindex=\"-1\" class=\"ngSelectionCheckbox\" type=\"checkbox\" ng-disabled=\"row.entity.default_value\" ng-checked=\"row.selected && !row.entity.default_value\" id=\"chk_{{ col.colDef.IDPreifx }}_{{ row.rowIndex }}\" /></div>"
  );
  ...
  $templateCache.put('footerTemplate.html',
    ...
    "    </div>\r" +
    "\n" +
    "    <div class=\"ngPagerContainer\" style=\"float: right; margin-top: 10px;\" ng-show=\"enablePaging && !pagingOnTop\" ng-class=\"{'ngNoMultiSelect': !multiSelect}\">\r" +
    "\n" +
  ...
  $templateCache.put('gridTemplate.html',
    ...
    "    <div class=\"ngPagerContainer\" style=\"float: right; margin-top: 0px;\" ng-show=\"enablePaging && pagingOnTop\" ng-class=\"{'ngNoMultiSelect': !multiSelect}\">\r" +
    "\n" +
    "        <div style=\"float:left; margin-right: 10px;\" class=\"ngRowCountPicker\">\r" +
    "\n" +
    "            <span style=\"float: left; margin-top: 3px;\" class=\"ngLabel\">{{i18n.ngPageSizeLabel}}</span>\r" +
