...
proto_ncm_setup() {
	...
	local device apn auth username password pincode delay mode pdptype profile metric $PROTO_DEFAULT_OPTIONS
	json_get_vars device apn auth username password pincode delay mode pdptype profile metric $PROTO_DEFAULT_OPTIONS
	...
	*)
	...
	if [ -z "$apn" ];then
		`gcom info -d $device > /tmp/stats/ncm_manufacture` >/dev/null 2>&1
		apn=`cat /tmp/stats/ncm_manufacture | grep APN: | cut -d , -f 3 | sed -e 's/"//g'`
		[ -z "$apn" ] && apn="internet"
		`rm /tmp/stats/ncm_manufacture`
	fi
	...
	manufacturer=`gcom -d "$device" -s /etc/gcom/getcardinfo_ncm.gcom | awk 'NF && $0 !~ /AT\+CGMI/ { sub(/\+CGMI: /,""); print tolower($1); exit; }'`
	[ $? -ne 0 ] && {
