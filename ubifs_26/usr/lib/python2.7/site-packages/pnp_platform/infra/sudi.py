...
################################################################################
"""PnP Infra SUDI API "implementation"

Since the reference platform does not have the hardware support for SUDI, this
implementation serves as placeholder for when manufacture-included certs are
a supported source of SUDI.

"""


# Note: pnp module will reject this API implementation since it does not inherit
# from PnPInfra
class _SUDI(object):
    """PnP Infra API: SUDI
    ...
    def sudi(cls):
        ...
        the first call to get_sudi should store the sudi value in the class
        attribute.
