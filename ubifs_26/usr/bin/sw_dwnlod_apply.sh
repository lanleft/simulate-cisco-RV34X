...
sendEmailAndLogAlert() {
    ...
    if [ "$status" = 0 ];then
	  ...
	  if [ "$IS_MR1" = "1" ]; then
            APPLIED_VER=`grep sec_latest_version "/etc/swupdateinfo" | awk -F'=' '{ print $2 }' |  sed 's/"//g'`
          else
...
if [ "$SWTYP" = "firmware" ];then
	...
	if [ "$IS_MR1" = "1" ]; then
	AVAILABLE_VER=`grep sec_latest_version "/etc/swupdateinfo" | awk -F'=' '{ print $2 }' |  sed 's/"//g'`
	else
