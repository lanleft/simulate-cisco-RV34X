...
wl_stats () {
    ...
    display_cmd_op "PKTQSTATS: wl -i $IFNAME pktq_stats" "$WLCMD -i $IFNAME pktq_stats c:// a: m: p: n:"
    display_cmd_op "RXREPORT: wl -i $IFNAME rx_report" "$WLCMD -i $IFNAME rx_report -noidle"
    display_cmd_op "AMT TABLE DUMP: wl -i $IFNAME dump amt" "$WLCMD -i $IFNAME dump amt"
    ...
    display_cmd_op "RESET COUNTERS: wl -i $IFNAME reset_cnts" "$WLCMD -i $IFNAME reset_cnts" "reset_cnts"
    ...
    display_cmd_op "SWKEYS: wl -i $IFNAME dump swkeys" "$WLCMD -i $IFNAME dump swkeys"
    display_cmd_op "HWKEYS: wl -i $IFNAME dump hwkeys" "$WLCMD -i $IFNAME dump hwkeys"
    ## Flush ceventc log
