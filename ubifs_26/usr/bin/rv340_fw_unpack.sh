...
PKG_MD5_ENCODED="fwcode"
PREUPGRADEFILE_MD5_ENCODED="preupgradecode"
...
if [ ! -f "$PKG_MD5_ENCODED" ];
then :
	logger -t system -p local0.info "$PKG_MD5_ENCODED not exist"
	# logger -t system -p local0.info "#Passing md5sum for legacy image verification!"
	# cp md5sum_fw-rv340 $PKG_MD5_ENCODED
fi


result="`/usr/bin/verify fw.gz $PKG_MD5_ENCODED`" > /dev/null 2>&1
[ "$result" != "0" ] && {
  logger -t system -p local0.error "Failed : verification of image failed!"
  logger -t system -p local0.error "Firmware Update Failed!"
  echo "Failed : verification of image failed!"
  echo "Firmware Update Failed!"
  pgrep -f "rv340_led.sh diag slow" | xargs kill -9 > /dev/null 2>&1
  rv340_led.sh diag fast > /dev/null 2>&1 &
  rm -rf $DIR_FIRMWARE* > /dev/null 2>&1
...
if [ -e "$PREUPGRADEFILE" ] && [ -e "$PREUPGRADEFILE_MD5" ]; then
	...
	if [ ! -f "$PREUPGRADEFILE_MD5_ENCODED" ];
	then :
		logger -t system -p local0.info "$PREUPGRADEFILE_MD5_ENCODED not exist"
		# logger -t system -p local0.info "##Passing md5sum for legacy image verification!"
		# cp $PREUPGRADEFILE_MD5 $PREUPGRADEFILE_MD5_ENCODED
	fi

	result="`/usr/bin/verify $PREUPGRADEFILE $PREUPGRADEFILE_MD5_ENCODED`" > /dev/null 2>&1
	[ "$result" != "0" ] && {
	        logger -t system -p local0.error "Failed : verification mismatched for $PREUPGRADEFILE"
		pgrep -f "rv340_led.sh diag slow" | xargs kill -9 > /dev/null 2>&1
		rv340_led.sh diag fast > /dev/null 2>&1 &
		rm -rf $DIR_FIRMWARE* > /dev/null 2>&1
		cd - > /dev/null 2>&1
